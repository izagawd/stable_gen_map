...
use crate::key::{Key, KeyData};
    ...
    fn clone_empty_map() {
        ...
        // lookups on empty clone should be None
        let dummy_key = DefaultKey::from(KeyData { generation: 0, idx: 0 });
        assert!(clone.get(dummy_key).is_none());
    }

    #[test]
    #[ignore]
    fn clone_preserves_values_and_keys() {
        let map = StableGenMap::<DefaultKey, i32>::new();
        ...
        // Same keys usable on the clone
        assert_eq!(clone.get(k1).copied(), Some(10));
        assert_eq!(clone.get(k2).copied(), Some(20));
        assert_eq!(clone.get(k3).copied(), Some(30));
    }
    ...
    fn clone_is_deep_not_shallow() {
        ...
        let (k, original_ref) = map.insert(Box::new("hello".to_string()));
        let clone = map.clone();
        let cloned_ref = clone.get(k).unwrap();

        // Values equal...
        assert_eq!(original_ref, "hello");
        assert_eq!(cloned_ref, "hello");

        // ...but not the same allocation (deep clone)
        let p1 =  original_ref;
        let p2 = cloned_ref;
        assert!(!ptr::eq(p1, p2));
    }
    ...
    fn clone_independent_mutation() {
        ...
        // Mutate the *clone*
        assert_eq!(clone.remove(k1).map(|b| *b), Some(1));
        let (k3_clone, _) = clone.insert(Box::new(3));

        // Original unaffected
        assert_eq!(map.len(), 2);
        assert_eq!(map.get(k1).copied(), Some(1));
        assert_eq!(map.get(k2).copied(), Some(2));
        assert!(map.get(k3_clone).is_none());

        // Clone has its own state
        assert_eq!(clone.len(), 2);
        assert!(clone.get(k1).is_none());
        assert_eq!(clone.get(k2).copied(), Some(2));
        assert_eq!(clone.get(k3_clone).copied(), Some(3));
    }

    #[test]
    #[ignore]
    fn clone_preserves_generations_and_free_list_behavior() {
        let mut map = StableGenMap::<DefaultKey, i32>::new();

        // Insert, then remove, then insert again to force generation bump / free reuse.
        let (k1, _) = map.insert(Box::new(10));
        let (k2, _) = map.insert(Box::new(20));

        assert_eq!(map.remove(k1).map(|b| *b), Some(10));

        let (k3, _) = map.insert(Box::new(30)); // likely reuses freed slot w/ bumped gen

        // Clone the state *after* the reuse.
        let mut clone = map.clone();

        // In the original:
        assert!(map.get(k1).is_none());
        assert_eq!(map.get(k2).copied(), Some(20));
        assert_eq!(map.get(k3).copied(), Some(30));

        // In the clone, same expectations:
        assert!(clone.get(k1).is_none());
        assert_eq!(clone.get(k2).copied(), Some(20));
        assert_eq!(clone.get(k3).copied(), Some(30));

        // Now mutate the clone further to ensure its free list works independently.
        assert_eq!(clone.remove(k2).map(|b| *b), Some(20));
        let (k4_clone, _) = clone.insert(Box::new(40));

        assert!(map.get(k4_clone).is_none()); // key from clone shouldn't exist in original
        assert_eq!(clone.get(k4_clone).copied(), Some(40));
    }