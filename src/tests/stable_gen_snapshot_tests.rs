...
fn snapshot_covers_all_items_and_matches_get() {
    ...
    for (k, v) in &snap {
        ...
        let from_map = map
            .get(*k)
            .expect("key from normal_map snapshot not found in map");
...
fn snapshot_refs_contains_all_values() {
    ...
    let from_pairs: HashSet<*const i32> = pairs.iter().map(|(_, v)| *v as *const i32).collect();
    let from_refs: HashSet<*const i32> = refs.iter().map(|v| *v as *const i32).collect();
